...
This file contains solutions to the CSV basics exercise.
"""
...
def transform_csv_data(input_path, output_path, transform_func):
    ...
    with open(input_path, 'r', newline='') as input_file:
        ...
        # Transform all rows
        transformed_rows = [transform_func(row) for row in csv_reader]
        
        # If no rows, return early
        if not transformed_rows:
            return 0
        
        # Get fieldnames from the first transformed row
        fieldnames = transformed_rows[0].keys()
        
        with open(output_path, 'w', newline='') as output_file:
            # Create writer with new fieldnames
            csv_writer = csv.DictWriter(output_file, fieldnames=fieldnames)
